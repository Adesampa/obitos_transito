...
grafico_interativo <- ggplot(dados_filtrados, aes(x = Ano, y = n, text = paste0(
  "Ano: ", Ano, "<br>",
  "Mortes: ", n, "<br>",
...
```{r echo=FALSE,  fig.height= 9, fig.width=18, out.width="100%"}
...
title_text <- "Evolução das Mortes no Trânsito por Meio de Locomoção da Vítima"

# Criar gráfico
  ggplot(dados_locomocao, 
                               aes(fill = locomocao, values = n)) +  # 🔹 Removi `text =` daqui
  geom_waffle(color = "white", size = .10, n_rows = 12, flip = TRUE) +
  ...
  theme(
    ...
    legend.text = element_text(family = body_font, size = 12, color = "grey10"),
    
    # Texto
    plot.title = element_text(family = "Lora", size = 12, face = "bold", hjust = 0.5, colour = "black"),
    plot.subtitle = element_text(family = "Lora", size = 14, hjust = 0.5, colour = "gray30"),
    text = element_text(family = "Lora"),
...
grafico_interativo <- ggplot(dados_locomocao, aes(x = Ano, y = n, color = locomocao, 
  ...
  MetBrewer::scale_fill_met_d("Redon", direction=1) +
  theme_minimal() +
  theme(plot.title = element_text(family = "Lora", size = 9, face = "bold", hjust = 0.5, colour = "black"),
        text = element_text(family = "Lora"),
