...
shp_distritos_2 <- shp_distritos %>% mutate(
    Indice = as.character(row_number()),
    ds_nome = str_trim(ds_nome),  
    ds_nome = stri_trans_general(ds_nome, "Latin-ASCII"), 
    ds_nome = str_to_title(ds_nome)  
...
shp_distritos_4 <- shp_distritos_3 %>%
  mutate(group_mortes = cut(mortes_cem_mil_hab, 
                            breaks = quantile(mortes_cem_mil_hab, probs = seq(0, 1, 0.2), na.rm = TRUE), 
                            include.lowest = TRUE))

# Criar o mapa com os distritos de São Paulo e a taxa de óbitos
pmap <- ggplot(shp_distritos_4) +
  geom_sf_interactive(aes(fill = mortes_cem_mil_hab, geometry = geometry, 
                          data_id = ds_nome, 
...
# Criar um gráfico interativo com ggiraph
interactive_plot_2_pedestre <- girafe(
  ggobj = pmap,
  width_svg = 14,  
  height_svg = 12,  
  options = list(
...
shp_distritos_4 <- shp_distritos %>% rename(ds_nome = NOME_DIST)

shp_distritos_5 <- shp_distritos_4 %>% mutate(
    Indice = as.character(row_number()),
    ds_nome = str_trim(ds_nome),  
    ds_nome = stri_trans_general(ds_nome, "Latin-ASCII"), 
    ds_nome = str_to_title(ds_nome)  
...
shp_distritos_6 <- shp_distritos_5 %>%
  left_join(dados_1, by = "ds_nome") %>% 
   mutate(n = replace_na(n, 0))

shp_distritos_4 <- shp_distritos_6 %>%
  mutate(group_mortes = cut(mortes_cem_mil_hab, 
                            breaks = quantile(mortes_cem_mil_hab, probs = seq(0, 1, 0.2), na.rm = TRUE), 
                            include.lowest = TRUE))
...
htmltools::browsable(
  tags$div(
    ...
    # Mapa interativo (primeiro gráfico)
    tags$div(style = "width: 50%;", interactive_plot_2_pedestre),  
...
scale_fill_fermenter(
    name = "Óbitos por 100k",
    breaks = c(0, 5, 10, 15, 20, 25, 30, ,35),  # Definindo intervalos menores
    direction = 1,
    palette = "Reds"
)+
  theme_map() +
...
pmap <- pmap + theme(
  ...
  annotate("text", 
           x = mean(st_bbox(shp_distritos_3)[c("xmin", "xmax")]),  # Centraliza no eixo X
           y = st_bbox(shp_distritos_3)["ymax"] + 0.05 * diff(st_bbox(shp_distritos_3)[c("ymin", "ymax")]),  # Move para cima
           label = "Mortes de condutores de automóvel \n p/ 100k hab.",
           hjust = 0.5, vjust = 1, size = 10, fontface = "bold", family = "Lora")
...
pmap <- ggplot(shp_distritos_4) +
  ...
  scale_fill_fermenter(
    name = "Óbitos por 100k",
    breaks = quantile(log1p(shp_distritos_3$mortes_cem_mil_hab), probs = seq(0, 1, 0.2), na.rm = TRUE),
    direction = 1,
    palette = "Reds"
  ) +
  theme_map() +
